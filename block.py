...
__all__ = ['C3k2_ECA','C3k2_CBAM','DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'CSP_MSCB_SC', 'EUCB_SC', 'C3k2_FAT', 'A2C2f_FMFFN', 'A2C2f_FMFFN_DYT', 'C3k2_LEGM', 'C3k2_RCB'
           ]
...
class ChannelAttention(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        # 使用全局平均池化和最大池化来聚合空间信息
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)

        # 一个共享的多层感知机（MLP），用于计算注意力权重
    ...
    def forward(self, x):
        ...
        # 将两条路径的结果相加，并通过Sigmoid函数得到通道注意力权重
...
#         super().__init__()
#
#         # -------- Direction-aware pooling (same idea) --------
#         self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
#         self.pool_w = nn.AdaptiveAvgPool2d((1, None))
#
#         # -------- Spatial interaction (replacing group conv1x1) --------
...
#         )
#
#         # -------- Local feature extraction (replacing group conv3x3) --------
#         self.conv_local = nn.Sequential(
#             nn.Conv2d(
#                 channels, channels,
#                 kernel_size=3, padding=1,
#                 groups=channels,  # depthwise = implicit group
#                 bias=False
#             ),
#             nn.BatchNorm2d(channels)
#         )
#
#         # -------- Global context (replacing agp + softmax) --------
...
#
#         # Spatial attention
#         spatial_attn = self.conv_hw(x_h + x_w)
#
#         # Local feature
#         local_feat = self.conv_local(x)
#
#         # Global attention
#         global_attn = self.conv_global(self.global_pool(x))
#
#         # Final attention fusion
#         attn = spatial_attn * global_attn
#
#         return x * attn + local_feat
...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        # 方向感知池化 + 带状卷积增强
        # x_h 形状: [B, C, H, 1] -> 经过 (k, 1) 卷积增强垂直特征
        x_h = self.strip_conv_h(self.pool_h(x))
        # x_w 形状: [B, C, 1, W] -> 经过 (1, k) 卷积增强水平特征
        x_w = self.strip_conv_w(self.pool_w(x))

        # 空间注意力融合 (广播机制相加)
        # 此时 x_h 和 x_w 的空间维度会自动对齐
        spatial_attn = self.conv_hw(x_h + x_w)
...
#
#             # 1. 坐标关注度（保留空间精确位置，对小目标至关重要）
#             self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
#             self.pool_w = nn.AdaptiveAvgPool2d((1, None))
#
#             # 2. 局部细粒度增强（使用更小分组，捕捉微小纹理）
#             self.conv_local = nn.Sequential(
#                 nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels, bias=False),
#                 nn.BatchNorm2d(channels)
#             )
#
#             # 3. 跨通道交互（采用类似Coordinate Attention的瓶颈结构）
#             self.conv1x1 = nn.Sequential(
#                 nn.Conv2d(channels, channels // 4, kernel_size=1, bias=False),
#                 nn.BatchNorm2d(channels // 4),
#                 nn.SiLU()  # 使用SiLU（Swish）相比Sigmoid对梯度更友好
#             )
#
#             self.conv_h = nn.Conv2d(channels // 4, channels, kernel_size=1, bias=False)
#             self.conv_w = nn.Conv2d(channels // 4, channels, kernel_size=1, bias=False)
#
#             self.sigmoid = nn.Sigmoid()
#
#         def forward(self, x):
...
#
#             # --- 局部特征分支 ---
#             local_f = self.conv_local(x)
#
#             # --- 坐标增强分支 (Coordinate-aware) ---
...
#             return out + local_f
    ...
    #     g = self.groups
    #
    #     # group split (structure-pruning friendly)
    #     x = x.view(b * g, -1, h, w)  # (b*g, c//g, h, w)
    #
    #     # H/W pooling
    #     x_h = self.pool_h(x)  # (b*g, c//g, h, 1)
    #     x_w = self.pool_w(x)  # (b*g, c//g, 1, w)
    #
    #     # broadcast + fusion
    #     x_hw = x_h + x_w  # implicit spatial alignment
    #     x_hw = self.conv_hw(x_hw)  # spatial gate
    #
    #     # local conv
    #     x_local = self.conv_local(x)
    #
    #     # attention map
    #     attn = self.conv_attn(x_hw * x_local)
    #
    #     # apply attention
    #     out = x * attn
    #
...
class Fusion(nn.Module):
    def __init__(self, inc_list, fusion='bifpn') -> None:
        ...
        elif self.fusion == 'AFusion':
            # 确保只有两个分支进行AFusion
            assert len(inc_list) == 2, "AFusion目前只支持两个分支的融合"

            # 第一个分支应用CBAM注意力
            self.cbam_branch1 = CBAM(inc_list[0])

            # 第二个分支应用EMA注意力
            self.ema_branch2 = EMA(inc_list[1])

            # 融合后的卷积层，将两个分支合并
            self.fusion_conv = nn.Sequential(
                nn.Conv2d(inc_list[0] + inc_list[1], max(inc_list), 1, padding=0),
                nn.BatchNorm2d(max(inc_list)),
                nn.SiLU(inplace=True)
            )
        elif self.fusion == 'gatedEMA':
            assert len(inc_list) == 2, "gatedEMA 只支持两分支融合"
            in_c = inc_list[0] + inc_list[1]  # 拼接后的通道
            self.ema = EMA(in_c)
    ...
    def forward(self, x):
        ...
        if self.fusion == 'weight':
            ...
            # 对第一个分支应用CBAM注意力[6,7](@ref)
            branch1_enhanced = self.cbam_branch1(x[0])

            # 对第二个分支应用EMA注意力
            branch2_enhanced = self.ema_branch2(x[1])

            # 将两个增强后的特征进行拼接融合[3](@ref)
            ...
            # 拼接两个分支
            branchall = torch.cat([x[0], x[1]], dim=1)

            # 通过 EMA 得到特征
            ema_feat = self.ema(branchall)

            # 得到 gate
            gate = torch.sigmoid(ema_feat)

            # gated 融合
            out = branchall * gate + ema_feat * (1 - gate)
...
class C3k_CBAM(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_CBAM(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class C3k2_CBAM(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k_CBAM(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_CBAM(self.c, self.c, shortcut, g) for _ in
            range(n))

#C3k2_DAA
class Bottleneck_DAA(nn.Module):
    """Standard bottleneck."""
...
class C3k_DAA(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_DAA(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class C3k2_DAA(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(
            C3k_DAA(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_DAA(self.c, self.c, shortcut, g) for _ in
            range(n))
...
class C3k2_ECA(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(
            C3k_ECA(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_ECA(self.c, self.c, shortcut, g) for _ in
            range(n))
#########################
...
class FocusFeature(nn.Module):
    def __init__(self, inc, kernel_sizes=(5, 7, 9, 11), e=0.5) -> None:
        ...
        # 2. 这里的对齐卷积非常棒，保持原样
        self.align_conv1 = Conv(hidc, hidc, 1)
        self.align_conv2 = Conv(hidc, hidc, 1)
        self.align_conv3 = Conv(hidc, hidc, 1)

        # 3. 关键：DW 卷积和 PW 卷积的通道数
        # 我们定义一个中间通道，让它在 cat 之后能被剪枝器追踪
        mid_c = hidc * 3

        # 使用 ModuleList 包装，但注意：
        # 为了让剪枝器识别 groups 需要同步修改，确保使用标准 nn.Conv2d
        self.dw_convs = nn.ModuleList([
            nn.Conv2d(mid_c, mid_c, kernel_size=k, padding=autopad(k),
                      groups=mid_c, bias=False)  # 剪枝器会自动修改这里的 groups
            for k in kernel_sizes
        ])
        self.dw_bns = nn.ModuleList([nn.BatchNorm2d(mid_c) for _ in kernel_sizes])

        self.pw_conv = Conv(mid_c, mid_c, 1)

    def forward(self, x):
        x1, x2, x3 = x
        x1 = self.align_conv1(self.conv1(x1))
        x2 = self.align_conv2(self.conv2(x2))
        x3 = self.align_conv3(self.conv3(x3))

        # 拼接后的通道数会自动成为 align_conv1+2+3 的输出通道之和
        x_cat = torch.cat([x1, x2, x3], dim=1)

        feature = x_cat
        for dw, bn in zip(self.dw_convs, self.dw_bns):
            # 这种写法会让剪枝器建立从 x_cat 到 dw 再到 bn 的强依赖
            feature = feature + bn(dw(x_cat))

        feature = self.pw_conv(feature)
        return x_cat + feature

######################################## Fo00cus Diffusion Pyramid Network end ########################################

# class FocusFeature(nn.Module):
#     def __init__(self, inc,  kernel_sizes=(5, 7, 9, 11), e=0.5) -> None:
#         """
#         inc: 输入通道列表 [c0, c1, c2]
#         out_ratio: 控制拼接特征通道倍数（原来固定是3）
#         kernel_sizes: 深度卷积核尺寸列表
#         e: hidc 缩放因子
#         """
#         super().__init__()
#         hidc = int(inc[1] * e)
...
#             self.conv3 = ADown(inc[2], hidc)
#         else:
#             self.conv3 = ADown(inc[1], hidc)
#         out_ratio = 3
#
#         # 深度卷积
#         self.dw_conv = nn.ModuleList(
#             nn.Conv2d(hidc * out_ratio, hidc * out_ratio, kernel_size=k, padding=autopad(k), groups=hidc * out_ratio)
#             for k in kernel_sizes
#         )
#
#         # 逐点卷积
#         self.pw_conv = Conv(hidc * out_ratio, hidc * out_ratio)
#
#     def forward(self, x):
#         if len(x) == 3:
#             x1, x2, x3 = x
...
#             x1, x2 = x
#             x2 = self.conv2(x2)
#             # 根据 x1 尺寸选择 conv1 或 conv3
#             if x1.shape[-2] > x2.shape[-2]:  # 高度更大就上采样
#                 x1 = self.conv3(x1)
#             else:  # 否则下采样
#                 x1 = self.conv1(x1)
#             #print('1221')
#             #print(x1.shape)
#             #print(x2.shape)
#             x_cat = torch.cat([x1, x2,x2], dim=1)
#         else:
#             raise ValueError(f"Unexpected number of inputs: {len(x)}")
#
#         # 特征融合
#         feature = torch.sum(torch.stack([x_cat] + [layer(x_cat) for layer in self.dw_conv], dim=0), dim=0)
#         feature = self.pw_conv(feature)
#         x_out = x_cat + feature
#         #print(x_out.shape)
#         return x_out
...
#     def __init__(self, inc, kernel_sizes=(5, 7, 9, 11), e=0.5) -> None:
#         """
#         inc: 3 个特征图通道 [c1, c2, c3]
#         kernel_sizes: depthwise 卷积 kernel list
#         e: 缩放比例
#         """
#         super().__init__()
#         self.e = e
...
#             Conv(inc[0], max(1, int(inc[1] * e)), 1)
#         )
#         self.reduce2 = Conv(inc[1], max(1, int(inc[1] * e)), 1) if e != 1 else nn.Identity()
#         self.reduce3 = ADown(inc[2], max(1, int(inc[1] * e)))
#
#         # depthwise 卷积模块列表（后续动态初始化）
#         self.dw_convs = nn.ModuleList()
#         self.pw_conv = None
#
#     def _init_dynamic_layers(self, in_channels):
#         """基于输入 cat 后的通道动态初始化 dw/pw"""
#         # depthwise: groups=in_channels
#         self.dw_convs.clear()
#         for k in self.ks:
#             self.dw_convs.append(
#                 nn.Conv2d(in_channels, in_channels, kernel_size=k,
#                           padding=autopad(k), groups=in_channels, bias=False)
#             )
#
#         # pointwise: 1x1 卷积，保持 in_channels -> in_channels
...
#
#         # 先做空间对齐
#         y1 = self.reduce1(x1)  # upsample -> conv
#         y2 = self.reduce2(x2)  # conv 或 identity
#         y3 = self.reduce3(x3)  # downsample -> conv
#
#         # cat 之后通道数
#         y = torch.cat([y1, y2, y3], dim=1)
#         C = y.shape[1]
#
#         # 第一次 forward 时动态初始化 dw/pw
#         if self.pw_conv is None or len(self.dw_convs) != len(self.ks):
#             self._init_dynamic_layers(C)
#
#         # 将 y 送入多个 depthwise，然后求和
#         outs = [y]  # identity
#         for dw in self.dw_convs:
#             # 如果剪枝后 dw 输入通道不匹配，会在这里报 size mismatch
#             if dw.in_channels != C:
#                 # 重新初始化所有 dw/pw 以匹配当前 C
#                 self._init_dynamic_layers(C)
#                 break
#             outs.append(dw(y))
#
#         # 重新 run 一次所有 dw
#         outs = [y] + [dw(y) for dw in self.dw_convs]
#
#         # 求和
#         feature = torch.stack(outs, dim=0).sum(dim=0)
#
#         # pointwise
#         feature = self.pw_conv(feature)
#
...
#         self.conv1 = nn.Sequential(
#             nn.Upsample(scale_factor=2),
#             Conv(inc[0], hidc, 1)  # 这里的 BN 可以被剪枝
#         )
#         self.conv2 = Conv(inc[1], hidc, 1) if e != 1 else nn.Identity()
#         self.conv3 = ADown(inc[2], hidc)
#
#         # -----------------------------------------------------------
#         # 【关键修改】添加一个过渡层 (Transition Layer)
#         # 它的作用是：无论前面 concat 出来是 384 还是 380，
#         # 它都能接收，并强行恢复成标准的 hidc * 3 输出。
#         # -----------------------------------------------------------
#         self.transition = Conv(hidc * 3, hidc * 3, 1)
#
#         # 下面的深度卷积
#         # 注意：剪枝脚本通常不会剪这个 transition 层的输出，
#         # 所以 dw_conv 接收到的永远是完整的 hidc * 3 通道。
#         self.dw_conv = nn.ModuleList(
#             nn.Conv2d(hidc * 3, hidc * 3, kernel_size=k,
#                       padding=autopad(k), groups=hidc * 3, bias=False)
#             for k in kernel_sizes
#         )
#         self.pw_conv = Conv(hidc * 3, hidc * 3)
#
...
#         x3 = self.conv3(x3)
#
#         # 1. 拼接：此时通道数可能会因为上游剪枝而变少
#         x_cat = torch.cat([x1, x2, x3], dim=1)
#
#         # 2. 过渡：利用标准卷积吸收通道变化
#         # 标准卷积允许 input_channels 变化 (只要切掉对应权重即可)
#         x_aligned = self.transition(x_cat)
#
#         # 3. 深度卷积：现在输入又是标准的 hidc*3 了，不会报错
#         feature = torch.sum(torch.stack([x_aligned] + [layer(x_aligned) for layer in self.dw_conv], dim=0), dim=0)
#
#         feature = self.pw_conv(feature)
#
#         # 注意：这里的残差相加可能会再次报错，因为 x_cat 和 feature 维度不同
#         # 如果需要残差，必须加在 transition 之后
#         return x_aligned + feature
